...
$(document).ready(function () {	
	...
	else
	{
		...
		//得到消息
		getAllMessageAll();
		setInterval("getAllMessage()",10000);	
...
function setHistoryMessage()
{
	...
	else
	{
		...
		if(l != null)
		{
			...
			{
				if((subjectCategory.is_p_audit ==1 && l.get(i).audit_status == 1) || subjectCategory.is_p_audit == 0)
				{
...
function isShowText()
{
	...
	{	
		$("#text_area_div").hide();
		$("#edit_div").hide();
		$("#pic_text_editor").css("line-height","24px");
...
function setVideoPath(live_video)
{
    ...
    if(live_video.indexOf(".flv")>0 || live_video.indexOf(".mp4")>0)
	{
...
function setOtherParam(m)
{
	if(m.get("other_set") != null)
	{
		...
		//李苏培加 -- start
		//alert(live_video);
		if(live_video=="undefined" || live_video==undefined){
		...
		if($("#live_video_div").attr("video_path") != null && $("#live_video_div").attr("video_path") != "")
		{
